...
The \emph{trajectory}\index{trajectory} of a body in a given reference
...
\begin{itemize}
  \item In the \emph{train} frame: the passenger moves at \qty{4}{km/h}
        down the aisle; the seats are at rest; the trees outside rush
        backward at \qty{300}{km/h}.
  \item In the \emph{ground} frame: the seats move at \qty{300}{km/h},
        the passenger at \qty{304}{km/h}, and the trees are at rest.
\end{itemize}
...
The state of rest or motion of a body, its trajectory, and its speed all
depend on the reference frame chosen to describe them.
\end{proposition}

\begin{proof}
One example settles each claim. The seated passenger of
\cref{ex:g10:relative-motion:walker} is at rest in the train frame and
moves at \qty{300}{km/h} in the ground frame (rest and speed depend on
the frame). For the trajectory, watch the valve on a bicycle wheel: an
...
curve called a \emph{cycloid}) --- see the figure below.
\end{proof}
...
{\small The same valve, two frames: a circle for the cyclist, a cycloid
...
reference frame, is the distance $d$ it travels in that frame divided by
the travel time $t$:
\[
v = \frac{d}{t} .
\]
With $d$ in metres and $t$ in seconds, $v$ is in metres per second
...
A hiker covers \qty{12}{km} in \qty{2}{h} of walking, then rests for
\qty{1}{h}, then covers \qty{6}{km} in the final \qty{1.5}{h}. Average
...
care about looks simplest. Three choices cover most of physics up to the
end of high school.
...
\begin{itemize}
  ...
  \item The \emph{geocentric frame}\index{geocentric frame} is centred
        ...
        for artificial satellites, which orbit the Earth as a whole and
  ...
  \item The \emph{heliocentric frame}\index{heliocentric frame} is
        ...
        a simple, nearly circular orbit.
\end{itemize}
\end{definition}
...
geocentric frame rides with the Earth without spinning, the heliocentric
...
\begin{enumerate}
  \item motion near the Earth's surface, lasting minutes --- ground
        frame;
  \item orbits \emph{around} the Earth (Moon, satellites) --- geocentric
        frame;
  \item orbits around the Sun (planets, comets) --- heliocentric frame.
\end{enumerate}
Always state your choice before describing a motion: a trajectory or a
...
the same observation reported in two frames --- the first in the ground
frame, where the Sun sweeps across the sky, the second in the geocentric
...
\qty{304}{km/h} (ground frame)? Along a single line, the answer is plain
addition.
...
metres along the line, and the body advances $v_{\text{body/support}}$
metres along the support. Both displacements happen along the same line,
...
belt --- at rest and in motion at the same time, in two different
frames.
\end{example}
...
its ground-frame trajectory is a slanted straight line. The full vector
treatment belongs to grade 11; here we only read the picture.
\end{remark}
...
goes straight across; in the ground frame it follows the slanted dashed
...
whether the passenger is at rest or in motion, and at what speed,
...
covers \qty{100}{m} in \qty{10.0}{s}. Compute both average speeds in
...
orbit of the International Space Station; one Martian year; a
\qty{100}{m} sprint.
\end{exercise}
...
\begin{enumerate}
  ...
  \item Compute the average speed over the round trip. Why is it less
...
A bicycle rolls along a straight road. Describe the trajectory of the
tyre valve in: the frame of the cyclist; the ground frame; and the frame
of the valve itself. Describe also the trajectory of the wheel's hub in
the ground frame. No computation --- name each curve.
\end{exercise}
...
once in \qty{24}{h}.
\begin{enumerate}
  \item Compute the speed of a point on the equator in the geocentric
...
back at \qty{90}{km/h}.
\begin{enumerate}
  \item Show that the average speed over the round trip is
...
\begin{enumerate}
  ...
  \item Using a right triangle, find the swimmer's speed in the ground
...
The Earth orbits the Sun on a near-circle of radius
\qty{1.50e11}{m}, in one year ($\num{3.16e7}$ seconds).
\begin{enumerate}
  \item Compute the Earth's speed in the heliocentric frame, in
        \unit{km/s}.
  ...
  \item Seen from the Earth, Mars occasionally seems to stop and drift
        ...
        sentences, using this chapter's vocabulary, why the heliocentric
...
\begin{enumerate}
  ...
  \item A child runs the wrong way at exactly \qty{1.0}{m/s} relative
        to the belt. Describe the child's motion in the ground frame,
        and name the gym machine this reinvents.
  \item Summarize questions 1--5 in a two-column table: each mover's
        speed in the belt frame and in the ground frame. Which column
...
\begin{enumerate}[resume]
  ...
  \item Using a right triangle, compute the ferry's speed in the ground
        frame and the length of its ground-frame trajectory. (A
        familiar triple of numbers appears.)
  \item Describe the ferry's trajectory in the water frame and in the
        ground frame. What is the same about them, and what differs?
  \item To land exactly opposite, the pilot aims upstream at an angle.
        Using a right triangle again: find the angle between the
        ferry's heading and the straight-across direction, the ferry's
        ground-frame speed, and the new crossing time. Why is landing
        on target slower than landing downstream?
  \item A log floats past the ferry. Give its speed in the ground frame
        and in the water frame. In which frame is the log ``moving''?
        Comment.
\end{enumerate}

\medskip
\noindent\textbf{Part III --- The satellite pass.}
At dusk, the International Space Station crosses the sky in a few
minutes. It orbits at an altitude of \qty{400}{km}; the Earth's radius
is \qty{6.37e6}{m}, and one orbit takes \qty{5.56e3}{s} (about
\qty{93}{min}).
\begin{enumerate}[resume]
  \item In which standard frame is the station's motion simplest?
        Compute its speed in that frame, in \unit{km/s}.
  \item Explain in two sentences why the ground frame describes the
  ...
  \item Compare the station's orbital speed with the equator's spin
        ...
        station faster?
  \item A geostationary satellite orbits at radius \qty{4.22e7}{m} in
        exactly one day ($\qty{8.64e4}{s}$). Compute its speed in the
        geocentric frame, then describe its motion in the ground frame.
        Reconcile the two answers in one sentence.
  \item In the geocentric frame the Moon traces a near-circle around
        the Earth in about $27$ days. Sketch in words its trajectory in
        the heliocentric frame over a few months.
\end{enumerate}
...
\begin{enumerate}[resume]
  ...
  \item The armchair traveller's speed report. You sit still in an
        armchair near the equator. Give your speed in the ground frame,
        in the geocentric frame ($\approx \qty{0.46}{km/s}$), and in
        the heliocentric frame ($\approx \qty{30}{km/s}$). Then compute
        the distance you cover in the heliocentric frame during a
