...
models of sources and \omterm{def:b1:dc-circuits:resistor}{resistors}, dividers, Th\'evenin and Norton, and
...
\index{voltage source}\index{current source}\index{electromotive force}\index{internal resistance}\index{Thevenin model@Th\'evenin model}\index{Norton model}
...
convention}, by
\[
u = E - r\,i \quad (\text{Th\'evenin model: ideal emf } E \text{ in series with } r)
\]
...
\begin{theorem}[Th\'evenin and Norton]\label{thm:b1:dc-circuits:thevenin}
\index{Thevenin's theorem@Th\'evenin's theorem}\index{Norton's theorem}
...
resistance $R_{\mathrm{Th}}$ (Th\'evenin), or a \omterm{def:b1:dc-circuits:sources}{current source} $I_N =
...
terminals, the rest of the circuit is a Th\'evenin source
...
= \qty{1.25}{V}$; total \qty{6.25}{V}. \emph{Th\'evenin} seen from
...
A non-linear dipole (characteristic $i = g(u)$) fed by a Th\'evenin
...
Find the Th\'evenin equivalent, seen from the \qty{2.2}{k\ohm}
...
Norton source to Th\'evenin and find the current in the \qty{2.0}{\ohm}
...
\begin{enumerate}
  \item Draw the \omterm{def:b1:dc-circuits:sources}{Th\'evenin model} and write $u(i)$ in the \omterm{thm:b1:dc-circuits:kirchhoff}{generator
        convention}.
