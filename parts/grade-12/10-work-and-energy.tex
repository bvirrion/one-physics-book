...
horizontal line (the marble's \omterm{def:g10:energy-conservation:energy}{energy}), no equation of motion solved.
Last year priced \omterm{def:g10:inertia:force}{forces} on straight lines (\cref{ch:g11:work-of-force})
and balanced kinetic against \omterm{def:g12:work-and-energy:ep}{potential energy}
...
Let a \omterm{def:g10:inertia:force}{force} $\vect F$ act on a body moving along any path from $A$ to
...
the small \omterm{def:g11:work-of-force:work}{work} $\scal{F}{\dd\ell} = F\,\dd\ell\cos\theta$
...
straight path with constant \omterm{def:g10:inertia:force}{force} collapses to last year's
...
$\vect{\dd\ell}$ earns $F\,\dd\ell\cos\theta$; the \omterm{def:g11:work-of-force:work}{work} is the sum.}
\end{omfigure}
...
$z_B$), the \omterm{def:g10:universal-gravitation:weight}{weight} of a mass $m$ does the \omterm{def:g11:work-of-force:work}{work}
...
A \omterm{def:g10:inertia:force}{force} is \emph{conservative}\index{conservative force} if its \omterm{def:g11:work-of-force:work}{work}
...
otherwise. \omterm{def:g10:universal-gravitation:weight}{Weight} and spring \omterm{def:g10:inertia:force}{force} (next section): conservative.
Sliding \omterm{def:g10:inertia:inventory}{friction} is not: opposite the motion, it does $W = -fL$ on a
path of length $L$ --- a toll per \omterm{def:g10:orders-of-magnitude:unit}{metre}, never refunded.
\end{definition}

\begin{example}[Two trails to the hut]\label{ex:g12:work-and-energy:trails}
A \qty{75}{kg} hiker \omterm{def:g10:signals-and-waves:oscilloscope}{gains} \qty{300}{m} by a steep \qty{800}{m} trail
or \qty{2.4}{km} of switchbacks. The \omterm{def:g10:universal-gravitation:weight}{weight} charges
$-75 \times 9.81 \times 300 \approx \qty{-2.2e5}{J}$ on both; a
\qty{30}{N} \omterm{def:g10:inertia:inventory}{friction} \omterm{def:g10:inertia:force}{force}, \qty{-2.4e4}{J} on one, \qty{-7.2e4}{J}
on the other. The \omterm{def:g10:universal-gravitation:weight}{weight} refunds on the descent; \omterm{def:g10:inertia:inventory}{friction}'s bill is
...
To every \omterm{def:g12:work-and-energy:conservative}{conservative force} one attaches a
\emph{potential energy}\index{potential energy!from conservative work}
$E_p$, a number depending on position alone, defined by
$W_{AB} = E_p(A) - E_p(B)$: \omterm{def:g11:work-of-force:work}{work} delivered is potential \omterm{def:g10:energy-conservation:energy}{energy} spent.
...
difference. For the \omterm{def:g10:universal-gravitation:weight}{weight} this gives back last year's $E_p = mgz$,
now valid along any path. \omterm{def:g10:inertia:inventory}{Friction} can own no $E_p$: a route-dependent
bill is no difference of endpoint numbers --- only \omterm{def:g12:work-and-energy:conservative}{conservative forces}
...
A spring of \omterm{def:g12:oscillators-and-time:spring}{stiffness} $k$ (\unit{N/m}) stretched or compressed by $x$
pulls back with the \omterm{def:g10:inertia:force}{force} $F = kx$
(\cref{ch:g12:oscillators-and-time}); the \omterm{def:g11:work-of-force:work}{work} it delivers returning
to rest --- its
...
\begin{proof}
The \omterm{def:g12:oscillators-and-time:pendulum}{restoring force} acts along the motion, so $\sum F\,\dd\ell$ is
the \emph{area under the $F$--$x$ graph}: a triangle of base $x$ and
height $kx$, area $\tfrac12 x \times kx$. The \omterm{def:g10:inertia:force}{force} at each stretch
is the same out and back: \omterm{def:g11:work-of-force:work}{work} fixed by the endpoints, spring \omterm{def:g10:inertia:force}{force}
\omterm{def:g12:work-and-energy:conservative}{conservative}, $E_p$ well defined.
\end{proof}
...
{\small The spring's \omterm{def:g10:inertia:force}{force} grows with the stretch; the stored \omterm{def:g10:energy-conservation:energy}{energy}
...
\]
the sum running over all \omterm{def:g10:inertia:force}{forces} acting on the body.
\end{theorem}
...
\omterm{thm:g12:newtons-laws:second}{Newton's second law} (\cref{ch:g12:newtons-laws}). In a short $\dd t$
the body moves $\vect{\dd\ell} = \vect v\,\dd t$: $E_k$ \omterm{def:g10:signals-and-waves:oscilloscope}{gains} the
small \omterm{def:g11:work-of-force:work}{work} of \cref{def:g12:work-and-energy:path}; summing the steps
sums the \omterm{def:g11:work-of-force:work}{works} --- last year's admitted statement, earned in full.
\end{proof}

\begin{proposition}[Instantaneous power]\label{prop:g12:work-and-energy:power}
At every instant a \omterm{def:g10:inertia:force}{force} $\vect F$ on a body of velocity $\vect v$
delivers the \omterm{def:g11:work-of-force:power}{power} $P = \scal{F}{v} = Fv\cos\theta$, and
$\dd E_k/\dd t$ is the sum of the \omterm{def:g11:work-of-force:power}{powers} of all \omterm{def:g10:inertia:force}{forces}: last year's
constant-velocity formula, exact at each instant of any motion.
\end{proposition}

\begin{proof}
Read off the previous proof: each \omterm{def:g10:inertia:force}{force} feeds
...
all the \omterm{def:g12:work-and-energy:conservative}{conservative forces} at \omterm{def:g11:work-of-force:work}{work}. Then, along any path,
\[
\Delta E_m = W_{AB}(\text{non-conservative forces}),
\]
and $E_m$ is conserved whenever the \omterm{def:g12:work-and-energy:conservative}{non-conservative forces} do no \omterm{def:g11:work-of-force:work}{work}.
\end{theorem}

\begin{proof}
Split the \omterm{def:g10:energy-conservation:energy}{kinetic-energy} theorem: $\Delta E_k = W_{\text{cons}} +
...
rest at $\qty{45}{\degree}$ from the vertical. The \omterm{def:g10:inertia:inventory}{tension},
perpendicular to the motion, is powerless; the \omterm{def:g10:universal-gravitation:weight}{weight} is \omterm{def:g12:work-and-energy:conservative}{conservative}:
...
\begin{enumerate}
  \item Choose the system and list the \omterm{def:g10:inertia:force}{forces} on it.
  \item Sort: perpendicular to the motion $\to$ no \omterm{def:g11:work-of-force:work}{work}; \omterm{def:g12:work-and-energy:conservative}{conservative}
        $\to$ an $E_p$ in $E_m$; the rest $\to$ $W_{\text{nc}}$
        (sliding \omterm{def:g10:inertia:inventory}{friction}: $-fL$, $L$ = path length).
  \item Write $\Delta E_m = W_{\text{nc}}$ between the data point and
        the question point; solve. \omterm{def:g10:energy-conservation:energy}{Energy} answers ``how fast, how
...
{\small An \omterm{def:g12:work-and-energy:diagram}{energy diagram}, read without solving anything: with \omterm{def:g10:energy-conservation:energy}{energy}
$E_m$ the body oscillates between $x_1$ and $x_2$, fastest where the
gap is largest; with $E_m'$ it crawls over the hill and \omterm{rem:g12:work-and-energy:escape}{escapes}.}
\end{omfigure}

\begin{proposition}[Force from the landscape; equilibria]\label{prop:g12:work-and-energy:equilibria}
The \omterm{def:g12:work-and-energy:conservative}{conservative force} along $x$ is minus the slope of $E_p$:
...
\begin{proof}
On a small step $\dd x$ the \omterm{def:g10:inertia:force}{force} \omterm{def:g11:work-of-force:work}{works} $F\,\dd x = E_p(x) -
...
reaches the ceiling never meets a \omterm{def:g12:work-and-energy:diagram}{turning point}: it
...
A spring of \omterm{def:g12:oscillators-and-time:spring}{stiffness} $k = \qty{200}{N/m}$ is stretched by
\qty{5.0}{cm}. Compute the stored \omterm{def:g10:energy-conservation:energy}{energy}; what does it become if the
...
$z = \qty{12}{m}$ to a balcony at $z = \qty{30}{m}$. \omterm{def:g11:work-of-force:work}{Work} of its
\omterm{def:g10:universal-gravitation:weight}{weight}? Along the straight climb? On the round trip?
\end{exercise}

\begin{exercise}[$\star$]\label{exo:g12:work-and-energy:3}
\omterm{def:g12:work-and-energy:conservative}{Conservative}, \omterm{def:g12:work-and-energy:conservative}{non-conservative}, or workless? Justify from paths:
(a) the \omterm{def:g10:universal-gravitation:weight}{weight}; (b) the spring \omterm{def:g10:inertia:force}{force}; (c) sliding \omterm{def:g10:inertia:inventory}{friction}; (d) the
\omterm{def:g12:newtons-laws:friction}{normal force} on a sliding box; (e) air drag.
\end{exercise}
...
point. Why does the wire's \omterm{def:g10:inertia:inventory}{tension} never enter the balance?
\end{exercise}

\begin{exercise}[$\star$]\label{exo:g12:work-and-energy:5}
A cyclist rides at \qty{9.0}{m/s} on the flat, delivering
\qty{250}{W}: what total \omterm{def:g11:work-of-force:sign}{resistive} \omterm{def:g10:inertia:force}{force} is she fighting? Climbing at
\qty{5.0}{m/s} with the same \omterm{def:g11:work-of-force:power}{power}: what \omterm{def:g10:inertia:force}{force} does she overcome?
\end{exercise}

\begin{exercise}[$\star\star$]\label{exo:g12:work-and-energy:6}
A toy launcher's spring ($k = \qty{450}{N/m}$) is compressed by
\qty{8.0}{cm} behind a \qty{20}{g} dart. Find the stored \omterm{def:g10:energy-conservation:energy}{energy}, the
...
Estimate the stored \omterm{def:g10:energy-conservation:energy}{energy} by trapezoidal areas; compare with
...
\qty{25}{m} curved run, arrives at \qty{9.0}{m/s}. \omterm{def:g11:mechanical-energy:em}{Mechanical energy}
lost? Average \omterm{def:g10:inertia:inventory}{friction} \omterm{def:g10:inertia:force}{force}? Why is the run's exact shape irrelevant?
\end{exercise}

\begin{exercise}[$\star\star$]\label{exo:g12:work-and-energy:9}
A marble track has a loop of radius \qty{20}{cm}. Find the minimum
release height (from rest, no \omterm{def:g10:inertia:inventory}{friction}). Released from \qty{60}{cm}:
...
A \qty{100}{g} bead slides without \omterm{def:g10:inertia:inventory}{friction} along an axis, with
$E_p(x) = 8.0\,x^2$ (\omterm{def:g11:work-of-force:work}{joules}, $x$ in \omterm{def:g10:orders-of-magnitude:unit}{metres}) and $E_m = \qty{2.0}{J}$.
\omterm{def:g12:work-and-energy:diagram}{Turning points}? Maximum speed? What kind of motion is this?
\end{exercise}

\begin{exercise}[$\star\star$]\label{exo:g12:work-and-energy:11}
A \qty{1200}{kg} car at \qty{2.0}{m/s} is stopped by a bumper spring
of \omterm{def:g12:oscillators-and-time:spring}{stiffness} $k = \qty{6.0e5}{N/m}$. Find the maximum \omterm{def:g12:sound-acoustics:sound-wave}{compression}.
At \qty{4.0}{m/s}: why not four times as much, the \omterm{def:g10:energy-conservation:energy}{energy} quadrupling?
\end{exercise}
...
weakening-gravity graph), compute the \emph{\omterm{rem:g12:work-and-energy:escape}{escape speed}} from Earth.
...
with \omterm{def:g12:oscillators-and-time:oscillator}{amplitude} \qty{6.0}{cm} (\cref{ch:g12:oscillators-and-time}).
Total \omterm{def:g10:energy-conservation:energy}{energy}? Maximum speed? Positions where $E_k = E_p$?
\end{exercise}
...
centre of mass \omterm{def:g10:signals-and-waves:oscilloscope}{gains} if the pole converts all his \omterm{def:g11:mechanical-energy:kinetic}{kinetic energy}, and
...
\begin{enumerate}
  ...
  \item Deduce the average \omterm{def:g11:work-of-force:sign}{resistive} \omterm{def:g10:inertia:force}{force} (snow plus air) over the run.
  \item What fraction of $E_m$ is lost? Why wax the skis and crouch?
\end{enumerate}

\medskip\noindent\textbf{Part II --- The flight.}
\begin{enumerate}[resume]
  \item Only the \omterm{def:g10:universal-gravitation:weight}{weight} acts in flight: which motion of
  ...
  \item Real jumpers ride the air like a wing, flying much farther:
        ...
        \omterm{def:g10:energy-conservation:energy}{mechanical-energy} theorem.
\end{enumerate}
...
\begin{enumerate}[resume]
  ...
  \item The legs absorb only the perpendicular part: compute that
        \omterm{def:g11:mechanical-energy:kinetic}{kinetic energy}; compare it with the total.
...
\begin{enumerate}[resume]
  ...
  \item Write the \omterm{def:g10:energy-conservation:energy}{mechanical-energy} theorem, gate to table, for $H'$.
  \item Solve for $H'$.
  \item By what percentage did \omterm{def:g10:inertia:inventory}{friction} raise the tower?
  \item The mass no longer cancels: recompute $H'$ for a
        \qty{60}{kg} jumper. Who needs the taller tower, and why?
  \item Punchline: the tower announced, and \omterm{def:g10:inertia:inventory}{friction}'s share of it.
\end{enumerate}
\end{problem}
