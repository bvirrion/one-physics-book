...
Les vitesses dépendent du \omterm{def:g10:relative-motion:frame}{référentiel} et se composent par addition
...
corps sans \omterm{def:g10:inertia:force}{force} garde une vitesse constante~: le sol, à peu près, et
tout \omterm{def:g10:relative-motion:frame}{référentiel} en \omterm{def:g12:kinematics-2d:uniform}{mouvement rectiligne uniforme} par rapport à lui.
\end{definition}
...
dans un autre~: «~maintenant, partout~» dépend du \omterm{def:g10:relative-motion:frame}{référentiel}.
\end{proposition}
...
premier~; mais dans son \omterm{def:g10:relative-motion:frame}{référentiel} aussi les deux éclairs ont
...
\omterm{def:g10:relative-motion:frame}{référentiel}~; le désaccord, des nanosecondes pour un train, est
...
\omterm{def:g10:relative-motion:frame}{référentiel} dans lequel cette horloge se déplace mesure un intervalle
...
verticalement~; et --- l'étape cruciale --- le
...
vitesses quotidiennes, divergent à l'asymptote $v = c$
...
prédites. Les accélérateurs misent sur la \omterm{thm:g12:special-relativity:dilation}{dilatation du temps} chaque jour,
...
altitude, travaille dans l'autre sens et gagne~: la
...
Un corps de longueur $L_0$ dans son \omterm{def:g10:relative-motion:frame}{référentiel} de repos est mesuré
plus court le long de sa direction de mouvement dans un \omterm{def:g10:relative-motion:frame}{référentiel}
...
dans son propre \omterm{def:g10:relative-motion:frame}{référentiel}, mais l'atmosphère qui monte à sa
rencontre est contractée à $\qty{15}{km}/30 = \qty{500}{m}$, traversée
bien dans \qty{2.2}{\micro s} --- les deux \omterm{def:g10:relative-motion:frame}{référentiels} s'accordent
...
Raconter le voyage du muon depuis son propre \omterm{def:g10:relative-motion:frame}{référentiel}~: quelle
...
labo~? Combien dans le \omterm{def:g10:relative-motion:frame}{référentiel} de l'électron, et quelle
...
\begin{enumerate}[resume]
  ...
  \item Quelle distance (\omterm{def:g10:relative-motion:frame}{référentiel} Terre, en
        années-lumière) le demi-tour a-t-il
        atteint~?
