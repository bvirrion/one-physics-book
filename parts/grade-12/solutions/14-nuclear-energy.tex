...
$E_b/A \approx \qty{7.68}{MeV}$ per nucleon.
\end{solution}
...
About $7.1$, $7.7$, $8.8$ and \qty{7.6}{MeV} per nucleon; iron-56 is
the most bound. Uranium can release energy by fission, helium-4 and
carbon-12 by fusion --- both moves climb toward iron; iron itself has
nowhere to go.
\end{solution}
...
(\qty{7.1}{MeV} per nucleon); helium-3 sits far lower on the curve, so
the step releases much less.
\end{solution}

\begin{solution}{exo:g12:nuclear-energy:9}
(a) The neutron is neutral: no Coulomb barrier, fission fires cold.
...
avalanche: each fusion is bought by its own collision, and any loss of
confinement cools the plasma and puts the fire out.
\end{solution}
...
(a) It slows fission neutrons by collisions; slow neutrons are far
more likely to fission uranium-235, so the chain needs them. (b) They
absorb neutrons, holding the chain at one neutron per fission ---
...
Fissions double each generation, so after $n$ generations about $2^n$
...
$E \approx \qty{-16.6}{MeV}$ --- the split \emph{absorbs} energy.
Iron sits at the peak: fissioning or fusing it both go downhill in
binding, both cost energy. It is the ash of every nuclear fire.
\end{solution}
...
$E_b/A \approx \qty{7.59}{MeV}$. Repacking $235$ nucleons at
...
against \num{1.4e6} kilograms of coal: ten million to one.
\end{solution}
...
\qty{8.35e-27}{kg}$: $\num{2.82e-12}/\num{8.35e-27} \approx
\qty{3.4e14}{J/kg}$ --- the ladder's fusion rung.
...
\textbf{15.} Nothing multiplies: no neutron lights the next fusion,
and losing confinement cools the plasma and stops it within seconds.
...
fuel surrenders the same kilogram; they differ only in how much cargo
must carry it.
...
$\num{1.2e3}$ kilograms of uranium-235, or $\num{2.8e2}$ kilograms of
D--T drawn from a pool and a half of seawater. Under every ledger the
same entry: one kilogram of mass, gone. $E = mc^2$ does not care which
